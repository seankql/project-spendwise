...
export default function PlaidController() {
  async function getPlaidLinkTokenUseCase(userId, token) {
    return fetch(
      "https://api.sw.cscc09.rocks:3001/api/plaid/link_token?" +
        new URLSearchParams({
          userId: userId,
        }),
      {
  ...
  async function getPlaidLinkedStatusUseCase(userId, token) {
    return fetch(
      "https://api.sw.cscc09.rocks:3001/api/plaid/has_linked_plaid?" +
        new URLSearchParams({
          userId: userId,
        }),
      {
  ...
  async function exchangePlaidTokenUseCase(userId, public_token, token) {
    return fetch("https://api.sw.cscc09.rocks:3001/api/plaid/token_exchange", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
      ...
      fetch(
        "https://api.sw.cscc09.rocks:3001/api/plaid/transactions/sync?" +
          new URLSearchParams({
            userId: userId,
          }),
        {
