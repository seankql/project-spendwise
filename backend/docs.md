...
### Exchange public_token for access_token & Sync Transaction

- URL: `POST /api/plaid/token_And_Sync`
  - content-type: `application/json`
  - Description: Exchange the public_token for an access_token and store it in the database and Synchronize transactions for a user from their linked bank account.
  - Request body: object
