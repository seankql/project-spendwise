...
transactionsController.get(
  ...
  async (req, res) => {
    try {
      ...
      Sentry.captureException("error getting transactions " + err);
...
transactionsController.post("/", validateAccessToken, async (req, res) => {
  try {
    ...
    Sentry.captureException("error creating transaction " + err);
    return res.status(500).send("Internal Server error " + err);
  }
});
...
transactionsController.put(
  ...
  async (req, res) => {
    try {
      ...
      Sentry.captureException("error updating transaction " + err);
      return res.status(500).send("Internal Server error " + err);
    }
  }
);
...
transactionsController.delete(
  ...
  async (req, res) => {
    try {
      ...
      Sentry.captureException("error deleting transaction " + err);
      return res.status(500).send("Internal Server error " + err);
    }
  }
);
