...
accountsController.post(
  ...
  async (req, res) => {
    try {
      ...
      Sentry.captureException("error creating account " + err);
...
accountsController.put(
  ...
  async (req, res) => {
    try {
      ...
      Sentry.captureException("error updating account " + err);
      return res.status(500).send("Internal Server error " + err);
    }
  }
);
...
accountsController.delete(
  ...
  async (req, res) => {
    try {
      ...
      Sentry.captureException("error deleting account " + err);
      return res.status(500).send("Internal Server error " + err);
    }
  }
);
...
accountsController.get(
  ...
  async (req, res) => {
    try {
      ...
      Sentry.captureException("error fetching accounts " + err);
      return res.status(500).send("Internal Server error " + err);
    }
  }
);
